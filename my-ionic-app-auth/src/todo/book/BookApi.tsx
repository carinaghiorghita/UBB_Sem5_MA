...
const different = (book1: any, book2: any) => {
    if (book1.title === book2.title && book1.author === book2.author && book1.publishingDate === book2.publishingDate && book1.owned === book2.owned)
      return false;
    return true;
}
      ...
      for (const each of result.data) {
          await Storage.set({
            ...
            value: JSON.stringify({
              ...
              title: each.title,
              author: each.author,
              publishingDate: each.publishingDate,
              owned: each.owned,
    ...
    result.then(async result => {
      ...
      await Storage.set({
        ...
        value: JSON.stringify({
          ...
          title: one.title,
          author: one.author,
          publishingDate: one.publishingDate,
          owned: one.owned,
